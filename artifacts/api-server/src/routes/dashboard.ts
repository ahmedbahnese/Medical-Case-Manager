...
router.get("/quality/dashboard", async (req, res): Promise<void> => {
  ...
  const dischargeByReason = await db.select({ reason: medicalCasesTable.dischargeReason, count: count() }).from(medicalCasesTable).where(eq(medicalCasesTable.status, "discharged")).groupBy(medicalCasesTable.dischargeReason);
  const dischargeByDepartment = await db.select({ department: medicalCasesTable.departmentId, count: count() }).from(medicalCasesTable).where(eq(medicalCasesTable.status, "discharged")).groupBy(medicalCasesTable.departmentId).orderBy(sql`count(*) DESC`);
  const respirationDaily = await db.select({ period: sql<string>`date(${medicalCasesTable.admissionDate} / 1000, 'unixepoch')`, count: count() }).from(medicalCasesTable).where(sql`${medicalCasesTable.artificialRespiration} IN ('high_frequency','vent','cpap','hfnc','standby')`).groupBy(sql`date(${medicalCasesTable.admissionDate} / 1000, 'unixepoch')`).orderBy(sql`date(${medicalCasesTable.admissionDate} / 1000, 'unixepoch') DESC`).limit(31);
  const respirationWeekly = await db.select({ period: sql<string>`strftime('%Y-W%W', ${medicalCasesTable.admissionDate} / 1000, 'unixepoch')`, count: count() }).from(medicalCasesTable).where(sql`${medicalCasesTable.artificialRespiration} IN ('high_frequency','vent','cpap','hfnc','standby')`).groupBy(sql`strftime('%Y-W%W', ${medicalCasesTable.admissionDate} / 1000, 'unixepoch')`).orderBy(sql`strftime('%Y-W%W', ${medicalCasesTable.admissionDate} / 1000, 'unixepoch') DESC`).limit(12);
