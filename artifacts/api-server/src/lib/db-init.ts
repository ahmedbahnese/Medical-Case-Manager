...
export async function initDatabase(): Promise<void> {
  try {
    const ddl = [
      ...
      `CREATE TABLE IF NOT EXISTS waiting_cases (id INTEGER PRIMARY KEY AUTOINCREMENT, patient_name TEXT NOT NULL, age TEXT, diagnosis TEXT, notes TEXT, parent_phone TEXT, national_id TEXT, medical_report TEXT, medical_report_name TEXT, medical_report_data TEXT, care_type TEXT NOT NULL, central_room_required INTEGER NOT NULL DEFAULT 0, central_room_code TEXT, artificial_respiration TEXT NOT NULL DEFAULT 'no', section TEXT NOT NULL DEFAULT 'reception', status TEXT NOT NULL DEFAULT 'waiting', transfer_destination TEXT, created_at INTEGER NOT NULL DEFAULT (strftime('%s','now')*1000), updated_at INTEGER NOT NULL DEFAULT (strftime('%s','now')*1000))`,
      ...
      `CREATE TABLE IF NOT EXISTS ovr_reports (id INTEGER PRIMARY KEY AUTOINCREMENT, report_number TEXT NOT NULL UNIQUE, event_date INTEGER NOT NULL, event_time TEXT, department TEXT NOT NULL, location TEXT NOT NULL, event_type TEXT NOT NULL, patient_name TEXT, file_number TEXT, hospital_supervision TEXT, administrative_manager TEXT, severity TEXT NOT NULL DEFAULT 'no_harm', description TEXT NOT NULL, immediate_action TEXT, reporter_name TEXT NOT NULL, reporter_role TEXT, investigator_name TEXT, investigation_date INTEGER, status TEXT NOT NULL DEFAULT 'new', investigation_summary TEXT, root_cause TEXT, corrective_action TEXT, preventive_action TEXT, action_owner TEXT, due_date INTEGER, verification_notes TEXT, reviewed_by TEXT, reviewed_at INTEGER, closed_by TEXT, closed_at INTEGER, created_at INTEGER NOT NULL DEFAULT (strftime('%s','now')*1000), updated_at INTEGER NOT NULL DEFAULT (strftime('%s','now')*1000))`,
      ...
      `CREATE TABLE IF NOT EXISTS notifications (id INTEGER PRIMARY KEY AUTOINCREMENT, message TEXT NOT NULL, from_user TEXT NOT NULL, delivery TEXT NOT NULL DEFAULT 'announcement', tone TEXT NOT NULL DEFAULT 'single', tone_duration_ms INTEGER NOT NULL DEFAULT 180, audience TEXT NOT NULL DEFAULT 'selected', recipients_json TEXT NOT NULL DEFAULT '[]', read_by_json TEXT NOT NULL DEFAULT '[]', created_at INTEGER NOT NULL DEFAULT (strftime('%s','now')*1000))`,
    ];
    ...
    const ovrColumns = [
      ["patient_name", "TEXT"], ["file_number", "TEXT"], ["hospital_supervision", "TEXT"], ["administrative_manager", "TEXT"],
      ["reporter_name", "TEXT"], ["reporter_role", "TEXT"], ["investigator_name", "TEXT"], ["investigation_date", "INTEGER"], ["status", "TEXT NOT NULL DEFAULT 'new'"],
      ...
      ["verification_notes", "TEXT"], ["reviewed_by", "TEXT"], ["reviewed_at", "INTEGER"], ["closed_by", "TEXT"], ["closed_at", "INTEGER"],
    ] as const;
    for (const [column, definition] of ovrColumns) {
      try { await db.run(sql.raw(`ALTER TABLE ovr_reports ADD COLUMN ${column} ${definition}`)); } catch { /* already exists */ }
    }
    try { await db.run(sql.raw("ALTER TABLE waiting_cases ADD COLUMN transfer_destination TEXT")); } catch { /* already exists */ }
    try { await db.run(sql.raw("ALTER TABLE waiting_cases ADD COLUMN notes TEXT")); } catch { /* already exists */ }
    try { await db.run(sql.raw("ALTER TABLE notifications ADD COLUMN delivery TEXT NOT NULL DEFAULT 'announcement'")); } catch { /* already exists */ }
    try { await db.run(sql.raw("ALTER TABLE notifications ADD COLUMN tone TEXT NOT NULL DEFAULT 'single'")); } catch { /* already exists */ }
