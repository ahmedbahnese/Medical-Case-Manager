...
export async function initDatabase(): Promise<void> {
  try {
    const ddl = [
      ...
      `CREATE TABLE IF NOT EXISTS ovr_reports (id INTEGER PRIMARY KEY AUTOINCREMENT, report_number TEXT NOT NULL UNIQUE, event_date INTEGER NOT NULL, event_time TEXT, department TEXT NOT NULL, location TEXT NOT NULL, event_type TEXT NOT NULL, severity TEXT NOT NULL DEFAULT 'no_harm', description TEXT NOT NULL, immediate_action TEXT, reporter_name TEXT NOT NULL, reporter_role TEXT, status TEXT NOT NULL DEFAULT 'new', investigation_summary TEXT, root_cause TEXT, corrective_action TEXT, preventive_action TEXT, action_owner TEXT, due_date INTEGER, verification_notes TEXT, reviewed_by TEXT, reviewed_at INTEGER, created_at INTEGER NOT NULL DEFAULT (strftime('%s','now')*1000), updated_at INTEGER NOT NULL DEFAULT (strftime('%s','now')*1000))`,
