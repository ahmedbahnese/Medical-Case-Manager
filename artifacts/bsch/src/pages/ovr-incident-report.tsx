...
export default function OvrIncidentReportPage() {
  ...
  const [form, setForm] = useState({ eventDate: new Date().toISOString().slice(0,10), eventTime: new Date().toTimeString().slice(0,5), department: "", location: "", eventType: "", patientName: "", fileNumber: "", hospitalSupervision: "", administrativeManager: "", severity: "no_harm", description: "", immediateAction: "" });
  ...
  return <div className="container mx-auto p-4 max-w-4xl" dir="rtl">
    ...
    <Card><CardHeader><CardTitle>إرسال بلاغ OVR</CardTitle></CardHeader><CardContent className="grid gap-4 md:grid-cols-2">
      ...
      <div><Label>الإشراف (اختياري)</Label><Input value={form.hospitalSupervision} onChange={e=>set("hospitalSupervision",e.target.value)} placeholder="اكتب اسم المشرف إن وجد" /></div>
      <div><Label>المدير الإداري (اختياري)</Label><Input value={form.administrativeManager} onChange={e=>set("administrativeManager",e.target.value)} placeholder="اكتب الاسم إن وجد" /></div>
