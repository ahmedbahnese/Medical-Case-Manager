...
import { CalendarDays, ClipboardPlus, Clock3, Edit3, Hospital, Phone, Plus, Printer, Stethoscope, Trash2, UsersRound, Megaphone } from "lucide-react";
import { toast } from "sonner";

type Appointment = { id: number; clinicId: number; patientName: string; age?: string | null; phone?: string | null; appointmentDate: string; appointmentTime?: string | null; queueNumber: number; source: string; status: string; notes?: string | null };
...
export default function OutpatientClinics() {
  ...
  const printQueue = () => {
    const rows = allAppointments.map(a => `<tr><td>${a.queueNumber}</td><td>${a.clinicName}</td><td>${a.patientName}</td><td>${a.appointmentTime ?? "—"}</td><td>${sourceLabels[a.source] ?? a.source}</td><td>${statusLabels[a.status] ?? a.status}</td></tr>`).join("");
