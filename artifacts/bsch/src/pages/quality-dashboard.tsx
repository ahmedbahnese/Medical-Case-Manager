...
interface QualityStats {
  ...
  departmentVisits?: Array<{ departmentId: number; count: number }>;
...
export default function QualityDashboard() {
  ...
  return <div className="space-y-5 animate-in fade-in duration-300" dir="rtl">
    <div className="flex items-center justify-between gap-3"><div><h1 className="text-xl font-bold flex items-center gap-2"><ShieldCheck className="h-6 w-6 text-primary" />لوحة مسؤول الجودة</h1><p className="text-xs text-muted-foreground">{hospital_name} — OVR وCAPA والتردد</p></div><div className="flex gap-2"><Button variant="outline" size="sm" onClick={load}><RefreshCw className="h-4 w-4 ml-1" />تحديث</Button><Button size="sm" asChild><Link href="/incident-report">إدارة بلاغات OVR</Link></Button></div></div>
    ...
    <div className="grid gap-4 md:grid-cols-2"><Card><CardHeader><CardTitle className="text-base">تردد الأقسام</CardTitle></CardHeader><CardContent className="space-y-2">{(stats.departmentVisits ?? []).map(x => <div key={x.departmentId} className="flex justify-between border-b pb-1"><span>قسم رقم {x.departmentId}</span><Badge>{x.count}</Badge></div>)}{!(stats.departmentVisits ?? []).length && <p className="text-sm text-muted-foreground">لا توجد بيانات</p>}</CardContent></Card><Card><CardHeader><CardTitle className="text-base">تردد التنفس الصناعي</CardTitle></CardHeader><CardContent className="grid grid-cols-3 gap-2"><div><p className="text-xs text-muted-foreground">يومي</p><p className="text-xl font-bold">{periodRows.daily[0]?.count ?? 0}</p></div><div><p className="text-xs text-muted-foreground">أسبوعي</p><p className="text-xl font-bold">{periodRows.weekly[0]?.count ?? 0}</p></div><div><p className="text-xs text-muted-foreground">شهري</p><p className="text-xl font-bold">{periodRows.monthly.at(-1)?.count ?? 0}</p></div></CardContent></Card></div>
